...
if (!admin) {
  return res.status(404).json({
     ...
     * user.messId
     *
     * Store them as pending changes only.
     */
 ...
 *
 * Only requests belonging to the logged-in admin
 * will be returned.
 */
 ...
 * =========================================================
 *
 * PUT /api/admin/profile-update-requests/:studentId/approve
 */
...
export const approveProfileUpdate = async (
  ...
  try {
    ...
    user.hostelName = pending.hostelName;

    user.roomNumber = pending.roomNumber;
    ...
    /*
     * Move user to NEW mess
     */
    user.messId = pending.requestedMessId;
